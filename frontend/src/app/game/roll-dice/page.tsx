...
export default function RollDiceGamePage() {
  ...
  useEffect(() => {
    ...
    const websocket = new WebSocket(
      buildWsUrl(`/game/roll-dice/ws/${gameId}`),
        ...
        <CardContent className="space-y-4 text-center">
          ...
          {gameResult.winners.length === 0 ? (
            ...
            <div className="rounded-lg bg-emerald-100 px-4 py-3 text-emerald-700 text-sm">
              Winners:{" "}
              {gameResult.winners
                .map((w) => (w.id === user.id ? "You" : w.name))
                .join(", ")}{" "}
              <br />
              Each receives{" "}
              <span className="font-semibold">{gameResult.winAmount}</span>
            </div>
          )}
          <div className="flex flex-wrap justify-center gap-3">
            {/* <Button
              onClick={playAgain}
              className="bg-slate-900 hover:bg-slate-900/90 text-white"
            >
              Choose another game
            </Button>
