...
export default function MatchFixingGamePage() {
  ...
  const [gameStatus, setGameStatus] = useState<
    "setup" | "waiting" | "answering" | "finished"
  >("setup");
  ...
  useEffect(() => {
    ...
    const websocket = new WebSocket(
      buildWsUrl(`/game/match-fixing/ws/${gameId}`),
  ...
  const addQuestion = () => {
    if (!ws || newOptions.some((opt) => !opt.trim()) || newOptions.length < 2)
      return;
  ...
  const finishGame = () => {
    ...
    ws.send(
      JSON.stringify({ type: "finish_game", answerIndex: selectedResult })
    );
  };
      ...
      return (
        <div className="rounded-lg border border-dashed border-slate-200 bg-slate-50 px-4 py-6 text-center text-sm text-slate-600">
          <p className="font-semibold text-slate-700">
            Waiting for the host to add a question
          </p>
      ...
      <div className="space-y-5">
        <div className="space-y-4 rounded-lg border border-slate-200 bg-slate-50 p-4">
          <AddChoicesPanel
            ...
            questionLabel={
              roomTitle ||
              (roomId ? `Room ${roomId} question` : "Host question")
            }
  ...
  const renderWaitingContent = () => (
    <div className="rounded-lg border border-dashed border-slate-200 bg-slate-50 px-4 py-6 text-center text-sm text-slate-600">
      ...
      <p className="mt-1">
        Players are joining the room or waiting for the host to start
      </p>
    </div>
  );
      ...
      <Card className="border shadow-sm">
        <CardHeader>
          ...
          <CardDescription className="text-xs text-slate-500">
            Host is preparing {questions.length} question(s)
          </CardDescription>
        </CardHeader>
    ...
    return (
      <Card className="border shadow-sm">
        <CardHeader className="space-y-1">
          ...
          <CardDescription className="text-xs text-slate-500">
            {canResolve
              ? "Select the real answer before announcing winners"
              : "Waiting for players to answer or for the game to finish"}
          </CardDescription>
        </CardHeader>
          ...
          {canResolve && currentQuestion ? (
            <>
              <p className="text-sm text-slate-600">
                When you know the real outcome, pick the correct option to pay
                the winners
              </p>
              ...
              >
                {selectedResult >= 0
                  ? `Selected option ${selectedResult + 1}`
            ...
            <p className="text-sm text-slate-600">
              Start the game to let players answer, then return here to reveal
              the result
            </p>
          )}
        </CardContent>
        ...
        <CardHeader className="space-y-1 text-center">
          ...
          <CardDescription className="text-sm text-slate-500">
            {gameResult.winners.length > 0
              ? "Perfect answers take the prize"
      ...
      <div className="space-y-5">
        ...
        <Card className="border shadow-sm">
          <CardHeader>
            ...
            <CardDescription className="text-xs text-slate-500">
              See who joined and how much they bet
            </CardDescription>
          </CardHeader>
